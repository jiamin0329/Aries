...
#ifndef ARIES_GRIDHEXA_HPP
#define ARIES_GRIDHEXA_HPP

#include "Grid.hpp"

namespace ARIES
{
    class GridHexa: public Grid
    {
    public:
        GridHexa(unsigned long val_point_0, unsigned long val_point_1,
    ...
    private:
        static unsigned short d_face[6][4];			/*!< \brief Matrix to store the local nodes of all the faces. */
        static unsigned short d_neighborNode[8][3];	/*!< \brief Neighbor to a nodes in the element. */
        static unsigned short d_numNodeFace[6];		/*!< \brief Number of nodes of each face of the element. */
        static unsigned short d_numNeighborNode[8];	/*!< \brief Number of Neighbor to a nodes in the element. */
