...
#ifndef ARIES_PARALLELBUFFER_HPP
#define ARIES_PARALLELBUFFER_HPP

#include "AriesOMP.hpp"
...
namespace ARIES
{
    ...
    public:
         ...
         * number.
         */
        void SetPrefixString(const std::string& text) { d_prefix = text; }
         ...
         */
        void SetOutputStream1(std::ostream* stream) { d_ostream1 = stream; }
         ...
         */
        void SetOutputStream2(std::ostream* stream) { d_ostream2 = stream; }

        /**
         * Write a text string to the output stream.  Note that the string is
         * not actually written until an end-of-line is detected.
         */
        void OutputString(const std::string& text) { OutputString(text, static_cast<int>(text.length())); }
         ...
         * from streambuf).
         */
        std::streamsize Xsputn(const std::string& text, std::streamsize n);
#endif
#endif
    ...
    private:
        ...
        ParallelBuffer(const ParallelBuffer& other);

        // Unimplemented assignment operator.
        ParallelBuffer& operator = (const ParallelBuffer& rhs);
        ...
        ARIES_omp_lock_t d_lBuffer;  // OpenMP lock for buffer operations.
